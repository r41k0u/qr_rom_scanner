  ...
  <ion-page>
    ...
    <svg :viewBox="viewBox" class="overlay" v-if="sharedStates.continuousScan">
      <polygon v-bind:key="'polygon'+index" v-for="(barcodeResult,index) in barcodeResults"
        :points="getPointsData(barcodeResult)" class="barcode-polygon" />
      <text v-bind:key="'text'+index" v-for="(barcodeResult,index) in barcodeResults" :x="barcodeResult.x1"
        :y="barcodeResult.y1" fill="red" font-size="25">{{barcodeResult.barcodeText}}</text>
    </svg>
    ...
    async function onScanned (result: ScanResult) {
      if (result.results.length > 0 && scanned === false && sharedStates.continuousScan === false) {
        ...
        result.results.forEach(element => {
          let stripped_string = element.barcodeText.replace(/[\r\n]/gm, '');
          if (base64regex.test(stripped_string)) {
