...
class CPU {
    ...
    loadRom(unintarray) {
                let program = new Uint8Array(unintarray)
...
fun = function(){
    ...
    document.getElementById("gamepad").addEventListener("mouseup", (event) => {    
        if(event.target.id!="gamepad"){
            char=event.target.id.slice(3,4).charCodeAt(0)
            keyboard.virtualKeyUp(char);        
        }    
    });
    document.getElementById("gamepad").addEventListener("mouseover", () => {
        keyboard.keysPressed=[]    
});
document.getElementById("backsel").addEventListener('click', function handleChange(event) {
    if (event.target.id!="backsel"){
        document.getElementById("emulator").style.backgroundColor = event.target.id;

    }
});
document.getElementById("pixsel").addEventListener('click', function handleChange(event) {
    if (event.target.id!="pixsel"){
        bincol = event.target.id;
